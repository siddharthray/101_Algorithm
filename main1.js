...
var sumWithRoman = function (num, romanMap) {
  ...
  var remainingVal = 0;
  var romanRes = "";
  var descSort = Object.values(romanMap).sort(function (a, b) {
    return b - a;
  });
  var roman = Object.keys(romanMap).find(function (key) {
    return romanMap[key] === num;
  });
  if (roman) {
    return roman;
  }
  var _loop_1 = function (i) {
    if (num >= descSort[i]) {
      var roman_1 = Object.keys(romanMap).find(function (key) {
        return romanMap[key] === descSort[i];
      });
...
// console.log(intToNumeral(10010));

function intToNumeral2(num) {
  const romanPairs = [
    ["M", 1000],
    ["CM", 900],
    ["D", 500],
...
console.log(intToNumeral2(10010));
